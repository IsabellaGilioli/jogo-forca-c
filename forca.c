...
void desenhaforca() {
    ...
    printf("\n\n");
     ...
     for(int i = 0; i < strlen(palavrasecreta); i++) {
        ...
        if(acertou() || enforcou() || jachutou(palavrasecreta[i])) {
...
void adicionapalavra() {
    ...
    if (quer == 'S') {
        ...
        f = fopen("palavras.txt", "r+"); //leitura e escrita
         if (f == 0) { //se o ponteiro f for nulo, ou seja, não funcionou
        ...
        fseek(f, 0, SEEK_SET); //aponta para o começo do arquivo
        fprintf(f, "%d", qtd);
...
void escolhepalavra() {
    ...
    f = fopen("palavras.txt", "r"); //read; devolve um ponteiro de arquivo, veja cima
    if (f == 0) { //se o ponteiro f for nulo, ou seja, não funcionou
...
int acertou() {
    ...
    return 1; //todas as letras da palavra secreta foram chutadas e acertadas
}
